...
Home::Home(QWidget *parent, const QString &userMode, const QString &userName)
    : QDialog(parent)
    , ui(new Ui::Home)
    , m_userMode(userMode)
    , m_userName(userName)
...
void Home::setupUI()
{
    ...
    setupNavButton(ui->Menu, "View Menu", ":/icons/menu.svg");
    ...
    infoLayout->addLayout(addInfo("Phone:", "NUMBER_HERE"));
    infoLayout->addLayout(addInfo("Upcoming:", "#_of_Reservations"));
    infoLayout->addLayout(addInfo("Member Since:", "Month Year"));
...
void Home::on_Menu_clicked()
{
    ...
    // Hide pages
    if (reservationsPage) reservationsPage->hide();
    if (bookingPage) bookingPage->hide();
}
...
void Home::setupReservationsPage()
{
    ...
    // Only show stat cards for users with the manager role
    if(m_userMode == "manager"){
        // Create stats cards
...
void Home::loadUserReservations(const ReservationFilter& filter)
{
    ...
    for (auto it = tables.begin(); it != tables.end(); ++it) {
        ...
        for (const QDateTime& reservationTime : info.reservedTimes) {
            if (!filter(info, reservationTime)) continue;
            ...
            // Table Number
            QLabel* tableNum = new QLabel(QString("Table %1").arg(it.key().mid(5)));
            ...
            connect(cancelBtn, &QPushButton::clicked, this, [this, it, reservationTime]() {
                TableInfo& tableInfo = tables[it.key()];
